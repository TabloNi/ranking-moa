      ...
      <Card>
        <CardHeader className="flex-row items-center gap-3 border-none mb-0">
          <div className="flex-1">
            ...
            >
              {project?.status === "review" ? "검토 중" : project?.status === "completed" ? "완료됨" : project?.status === "in progress" ? "진행 중" : "준비 중"}
            </Badge>
          </div>
        ...
        <CardFooter className="flex justify-between border-t  p-4">
          ...
          <div>
            <div className="text-xs  text-default-600 mb-[2px]">종료일:</div>
            <span className="text-xs font-medium text-default-900">
              {project?.dueDate}
            </span>
          </div>
